...
// LoadImage decodes a static image (JPEG/PNG) and returns it as a single-frame GifAnimation
// so it can be rendered using the same GifSlot infrastructure.
func LoadImage(path string, heightPx int) (*GifAnimation, error) {
	f, err := os.Open(path)
	if err != nil {
	...
	// Square crop for circular mask
	dstH := heightPx
	dstW := heightPx

	frame := scaleToRGBACircular(img, dstW, dstH)

	return &GifAnimation{
		Frames: [][]byte{frame},
		Delays: []time.Duration{time.Hour}, // static image, never advances
		Width:  dstW,
		Height: dstH,
	}, nil
}
...
// a circular mask with anti-aliasing (same approach as the profile pic overlay).
